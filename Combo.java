...
public class Combo {
    ...
    protected Dictionnaire word;
    protected Case coord;
    ...
    public List<Assigment> getAssignmentList(Lettre alphabet) {
        ...
        for (int i = 0; i < array.length; i++) {
            result.add(new Assigment(x + i * dx, y + i * dy, alphabet.getSac(array[i])));
        }
        return result;
    }
    ...
    public String getString(Lettre alphabet) {
        return alphabet.toString(word);
    }

    public Dictionnaire getCoord() {
        return coord;
    }
    ...
    protected Combo(Dictionnaire word, Case coord, int credits) {
        this(word, coord, credits, OrientationType.HORIZONTAL);
    }

    protected Combo(Dictionnaire word, Case coord, int credits, OrientationType orientation) {
        this.word = word;
        this.coord = coord;
    ...
    protected Combo getMirror() {
        Case c = new Case(coord.y, coord.x);
