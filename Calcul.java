...
public class Calcul {
	...
	/* Chaque lettre du mot jouer est un char du coup je creer une classe avec char et son point pour faciliter le scoring*/
	/* */
	...
	//
	public int calculmot() //calcul du mot du tout court, ça ma servi àprés pour dans les case pour mot*3 etc...
	{
		int n = 0;
		lettre_Point();
	 ...
	 * */ 
	public int calculMot() // pour moi,renvoi un entier pour permettre de faire un cumul à chaque placement
	{
		int[][] t = new int[15][15];
		int n = 0;
		...
		for (int i = 0; i < mot.length(); i++) {
		    ...
		    if(t[valD[i][0]][valD[i][1]]==2){
		    	for (int j = 0; j < Let_P.size(); j++) {
					...
					{
					 r=1;
					 //au cas ou une autre1 lettre sur la case mot x2 ou 3
          ...
          //si la lettre est placé sur la case Mot x2
            if(t[valD[i][0]][valD[i][1]]==4){
            	if ((i+1)!=mot.length())//il permet de savoir si le mot devra à la fin etre *
            		{
                    m = 1;	
             	}
             	else // sinon si la derniere lettre est sur une case mot* il fait la somme et le *
             		{
             		for (int j = 0; j < Let_P.size(); j++) {
    					if(mot.charAt(i)==(Let_P.get(j).getLettre()))
    					{
	...
	public static void affTableauPoint(int[][] t) {
		tPoint(t);
		for (int i = 0; i < 15; i++) {
			for (int j = 0; j < 15; j++) {
				System.out.print("[" + t[i][j] + "]");
